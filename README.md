...
## 🚀 배포 및 트러블슈팅 (Troubleshooting)

### 💡 "Oracle 대신 MySQL을 선택한 이유" (AWS 배포 과정)
로컬에서 사용하던 **Oracle DB**를 클라우드에 올리면서 마주한 문제와 해결 과정입니다.

-   **문제 상황**: 프로젝트 배포를 위해 외부 DB가 필요했는데, Oracle Cloud 가입이 막히는 예상치 못한 상황이 발생했습니다. 😅
-   **해결 방법**: 대안으로 **AWS EC2 서버에 직접 MySQL을 설치**하여 DB 환경을 구축했습니다.
-   **왜 MySQL?**:
    -   **가벼움**: 프리티어 사양(`t2.micro`) 서버에서도 무리 없이 잘 돌아갑니다.
    -   **빠른 전환**: Oracle과 SQL 문법이 비슷해서 기존 코드를 거의 고치지 않고 빠르게 적용할 수 있었습니다.
    -   **직접 관리**: 서버에 직접 설치해 보며 DB 설정과 권한 관리를 경험해 볼 좋은 기회라고 생각했습니다.
